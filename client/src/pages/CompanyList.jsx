    ...
    try {
      ...
      setRows(
        (d.data || []).map((c) => ({
          ...
          totalPlaced: c.totalPlaced || 0,
          avgPlaced: c.avgPlacedPerDrive || 0,
          lastDriveDate: c.lastDriveDate
            ? new Date(c.lastDriveDate).toLocaleDateString()
            : '-',
        }))
      );
    } catch (error) {
      ...
      toast.error(
        'Load Failed',
        'Failed to load companies due to a network error'
      );
    } finally {
      setLoading(false);
    }
  ...
  async function deleteCompany(companyId, companyName) {
    ...
    try {
      ...
      if (response.ok) {
        toast.success(
          'Company Deleted',
          `${companyName} has been successfully deleted.`
        );
      ...
      toast.error(
        'Delete Failed',
        'Failed to delete company due to a network error'
      );
    }
  }
        ...
        >
          ...
          <Table
            columns={[
              ...
              { label: 'Total Placed', render: (r) => r.totalPlaced },
              { label: 'Avg Placed per drive', render: (r) => r.totalPlaced },
