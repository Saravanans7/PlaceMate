...
export default function CompanyDetail() {
  ...
  async function loadCompanyData() {
    ...
    try {
      ...
      const companyResponse = await fetch(
        `/api/companies?search=${encodeURIComponent(companyName)}`,
        { credentials: 'include' }
      );
      ...
      const foundCompany = (companyData.data || []).find(
      ...
      if (user?.role === 'staff') {
        ...
        const regResponse = await fetch(
          `/api/registrations?company=${foundCompany._id}`,
          { credentials: 'include' }
        );
          ...
          for (const reg of regData.data || []) {
            ...
            try {
              ...
              const studentsResponse = await fetch(
                `/api/registrations/${reg._id}/applicants`,
                { credentials: 'include' }
              );
              const studentsData = await studentsResponse.json();

              console.log(
                'Students response for registration',
                reg._id,
                ':',
                studentsResponse.ok,
                studentsData
              ...
              if (studentsResponse.ok) {
                const studentsWithRegInfo = (studentsData.data || []).map(
                  (student) => ({
                    ...student,
                    registrationInfo: {
                ...
                console.error(
                  'Failed to fetch students for registration:',
                  reg._id,
                  studentsData
                );
              }
              ...
              console.error(
                `Failed to load students for registration ${reg._id}:`,
                err
              );
            }
          }
        ...
        if (user?._id) {
          try {
            ...
            const regResponse = await fetch(
              `/api/registrations?company=${foundCompany._id}&status=open`,
              { credentials: 'include' }
            );
            const regData = await regResponse.json();
            if (regResponse.ok) {
              ...
              if (regs.length > 0) {
                ...
                if (appsResponse.ok) {
                  const appRegIds = new Set(
                    (appsData.data || []).map((a) =>
                      String(a.registration?._id || a.registration)
                    )
                  );
                  const hasApplied = regs.some((r) =>
                    appRegIds.has(String(r._id))
                  );
  ...
  async function manageRoundsForRegistration(registrationId) {
    try {
      const r = await fetch(`/api/drives/by-registration/${registrationId}`, {
        credentials: 'include',
      });
      ...
      <div className="container grid">
        <Card title={`${company.name} — ${company.role || ''}`}>
          <div className="company-info">
            ...
            <p>
              <b>Salary:</b> {company.salaryLPA || '-'} LPA
            </p>
            <p>
              <b>Last drive:</b>{' '}
              {company.lastDriveDate
                ? new Date(company.lastDriveDate).toLocaleDateString()
                : '-'}
            </p>
            {user?.role === 'staff' && (
              <>
                <p>
                  <b>Total Registrations:</b> {registrations.length}
                </p>
                <p>
                  <b>Total Registered Students:</b> {registeredStudents.length}
                </p>
              </>
            )}
          </div>
          <div className="cta">
            {user?.role === 'staff' && (
              <Link
                ...
                to={`/company/create-registration?companyId=${company._id}`}
              >
                Open Registration
              </Link>
            )}
            {user?.role === 'student' && !alreadyRegistered && (
              <Link
                className="btn btn-primary"
                to={`/company/${encodeURIComponent(company.name)}/register`}
              >
                Register
              </Link>
            )}
            ...
            {user?.role === 'student' && alreadyRegistered && (
              <Link
                className="btn btn-primary"
                to={`/drive/${encodeURIComponent(company.name)}`}
              >
                View Drive
              </Link>
            )}
        ...
        {user?.role === 'staff' && (
          <div>
                  ...
                  {registrations.map((reg) => (
                    <div key={reg._id} className="student-card">
                      <div className="student-header">
                        <h4>
                          Drive: {new Date(reg.driveDate).toLocaleDateString()}{' '}
                          • Batch {reg.batch}
                        </h4>
                      ...
                      <div className="registration-info">
                        <Button
                          onClick={() => manageRoundsForRegistration(reg._id)}
                        >
                          Manage Rounds
                        </Button>
                      </div>
            ...
            <Card
              title={`Registered Students (${registeredStudents.length})`}
              actions={
            ...
            >
              {registeredStudents.length === 0 ? (
                <div className="no-students">
                  ...
                  <p>
                    Students need to register for company drives to appear here.
                  </p>
                </div>
              ) : (
                <div className="students-list">
                  {registeredStudents.map((app, index) => (
                    <div key={`${app._id}-${index}`} className="student-card">
                      ...
                      <div className="student-details">
                        ...
                        {app.student.batch && (
                          <span>Batch: {app.student.batch}</span>
                        )}
                        {app.student.cgpa && (
                      ...
                      <div className="registration-info">
                        <span>
                          Drive Date:{' '}
                          {new Date(
                            app.registrationInfo.driveDate
                          ).toLocaleDateString()}
                        </span>
                        <span>Batch: {app.registrationInfo.batch}</span>
                        <span
                          className={`status-badge ${app.registrationInfo.status}`}
                        >
          ...
          <div>
            <Card title="Company Information">
              <div className="student-company-info">
                ...
                <p>
                  This company offers great opportunities for students in your
                  field.
                </p>
                <div className="company-stats">
                  <div className="stat-item">
                    ...
                    <span className="stat-value">
                      {company.salaryLPA || 'Not specified'} LPA
                    </span>
                  </div>
                  <div className="stat-item">
        ...
        <Card title="Rounds Template">
          {(company.roundsTemplate || []).map((r, i) => (
            <p key={i}>
              <b>Round {i + 1}:</b> {r.name} — {r.description || ''}
            </p>
          ))}
        </Card>
      </div>
